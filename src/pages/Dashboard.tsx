...
const Dashboard: React.FC = () => {
  ...
  const PatientDashboard = () => (
    <div className="space-y-6">
      {/* Quick Actions */}
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4 dark:text-gray-100">Quick Actions</h2>
          ...
          {quickActions.map((action, index) => (
            <Link key={index} to={action.href} className="group">
              <div className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                ...
                <h3 className="font-medium text-gray-900 mb-2 dark:text-white">{action.title}</h3>
            ...
            {therapistStats.schedule.length === 0 ? (
              ...
              therapistStats.schedule.map((session, idx) => (
                <div key={session._id || idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg dark:bg-gray-900">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium dark:text-gray-100">{session.sessionDate ? new Date(session.sessionDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--'}</p>
